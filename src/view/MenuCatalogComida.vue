...
const activeCategory = ref('') // Empezamos directo en Tortas
...
const filteredItems = computed(() => {
  ...
  return menuItems.value.filter(item => {
    // Excluir tortas (category_id 18) — comparar como string y número
    if (Number(item.category_id) === 18 || item.category?.name === 'Tortas') return false

    // Filtrar por categoría activa
    if (activeCategory.value && item.category?.name !== activeCategory.value) return false
...
const resetFilters = () => {
  searchQuery.value = ''
  activeCategory.value = dymanicCategories.value[0] ?? ''
}

const dymanicCategories = computed(() => {
  const cats = new Set(
    menuItems.value
      .filter(i => Number(i.category_id) !== 18 && i.category?.name !== 'Tortas')
...
onMounted(async () => {
  ...
  try {
    ...
    if (dymanicCategories.value.length > 0) {
      activeCategory.value = dymanicCategories.value[0] ?? ''
    }
  } catch (error) {
